...
/*******************************************************************************
* Function Name: Clock_lento_Start
********************************************************************************
*
* Summary:
...
void Clock_lento_Start(void) 
{
    ...
    Clock_lento_CLKEN |= Clock_lento_CLKEN_MASK;
	Clock_lento_CLKSTBY |= Clock_lento_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_lento_Stop
********************************************************************************
*
* Summary:
...
void Clock_lento_Stop(void) 
{
    ...
    Clock_lento_CLKEN &= (uint8)(~Clock_lento_CLKEN_MASK);
	Clock_lento_CLKSTBY &= (uint8)(~Clock_lento_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_lento_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_lento_CLKEN & Clock_lento_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_lento_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_lento_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_lento__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_lento_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_lento_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_lento_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_lento_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_lento_GetSourceRegister();
    uint16 oldDivider = Clock_lento_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_lento_CLKEN & Clock_lento_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_lento_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_lento_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_lento_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_lento_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_lento_MOD_SRC |= modeBitMask & (uint8)Clock_lento_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_lento_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_lento_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_lento_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_lento_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_lento_MOD_SRC & (uint8)(Clock_lento_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_lento_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_lento_GetDividerRegister();
    uint8 oldSrc = Clock_lento_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_lento_MOD_SRC |= CYCLK_SSS;
        Clock_lento_MOD_SRC =
            (Clock_lento_MOD_SRC & (uint8)(~Clock_lento_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_lento_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_lento_MOD_SRC & Clock_lento_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_lento_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_lento_PHASE = clkPhase & Clock_lento_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_lento_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_lento_PHASE & Clock_lento_PHASE_MASK;
}
